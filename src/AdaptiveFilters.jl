...
"""
    yh = adaptive_filter(y, loss=l2regloss, alg=MSPI; order=4, lr=0.1)
...
#Arguments:
- `loss`: Loss function. Examples: `l2regloss, l1regloss, logisticloss, l1hingeloss`
...
"""
function adaptive_filter(y, loss::Function=l2regloss, alg::Type{<:OnlineStats.Algorithm}=MSPI; order=4, lr=0.1)
    T = length(y)
    model = StatLearn(loss, order, alg(), rate=LearningRate(lr))
    yh = similar(y)
...
adaptive_filter(y, alg::Type{<:OnlineStats.Algorithm}; kwargs...) = adaptive_filter(y, l2regloss, alg; kwargs...)
